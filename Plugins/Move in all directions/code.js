...
function moveMapObj(id, dir, camera)
{
	...
	Core.MapObject.search(id, (result) =>
	{
		if (!!result)
		{
			...
			const dist = Math.min(1, result.object.speed.getValue() * Core.MapObject.SPEED_NORMAL * Manager.Stack.averageElapsedTime) * Data.Systems.SQUARE_SIZE;
			result.object.move(Common.ORIENTATION.SOUTH, dist * Math.cos(dir * Math.PI / 180), 270 + (camera ? angle : -90), camera);
			result.object.move(Common.ORIENTATION.SOUTH, dist * Math.sin(dir * Math.PI / 180), 180 + (camera ? angle : -90), camera);
			if (!result.object.currentStateInstance.directionFix)
			{
				if (camera)
					result.object.lookAt(mod(Math.round(180 - dir / 90) + Scene.Map.current.camera.getMapOrientation() - 3, 4));
				else
