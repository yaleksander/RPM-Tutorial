...
function evaluate(expr, t, r)
{
	switch (expr.token)
	{
		...
		case Token.SQR:   return Data.Systems.SQUARE_SIZE;
...
{
	if (object == -1)
		object = Core.ReactionInterpreter.currentObject.id;
	else if (object == 0)
		object = Core.Game.current.hero.id;
	for (var i = 0; i < emitterList.length; i++)
	{
		const e = emitterList[i];
		if (e.id === object)
...
Manager.Plugins.registerCommand(pluginName, "Start particle effect", (object, rate, lifespan, position, rotation, size, opacity, texture, additiveBlending) =>
{
	...
	if (object == -1)
		...
		object = Core.Game.current.hero.id;
	Core.MapObject.search(object, (result) =>
	{
