...
Scene.Map.prototype.updateCameraHiding = function (pointer)
{
	...
	const dir = (target ? this.camera.target.position : Core.Game.current.hero.position).clone().add(new THREE.Vector3(0, 0.5 * Data.Systems.SQUARE_SIZE, 0)).sub(this.camera.getThreeCamera().position);
	while (dir.y < 0)
	{
		dir.y += 0.01;
		...
		for (var i = 0; i < intersects.length; i++)
		{
			...
			if (Math.ceil(intersects[i].distance) + 5 < this.camera.distance)
			{
				...
				if ((v[eraseMount] && m) || (v[eraseWalls] && w) || (v[eraseObj3D] && o) || (e && intersects[i].object.viewBlockPlugin_erase) || (v[eraseFloor] && !w && !m && !e && !o))
				{
					...
					intersects[i].object.material.userData.uniforms.viewBlockPlugin_squareSize = {value: Data.Systems.SQUARE_SIZE};
