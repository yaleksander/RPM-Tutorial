...
async function init()
{
    vert = await Common.IO.openFile(path + "shaders/waterfall.vert");
    frag = await Common.IO.openFile(path + "shaders/waterfall.frag");
    shadersLoaded = true;
}
init();
...
Manager.Plugins.registerCommand(pluginName, "Create waterfall", (id, diameter, height, shape, speed, topDark, bottomDark, topLight, bottomLight, foamColor, addFoam, opacity) =>
{
    if (id == -1)
        id = Core.ReactionInterpreter.currentObject.id;
