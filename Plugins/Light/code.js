...
setInterval(function ()
{
	if (Manager.Stack.top instanceof Scene.Map && !Scene.Map.current.loading)
		for (var i = 0; i < Scene.Map.current.scene.children.length; i++)
			if (!!Scene.Map.current.scene.children[i].customDepthMaterial && !Scene.Map.current.scene.children[i].customDistanceMaterial)
				Scene.Map.current.scene.children[i].customDistanceMaterial = new THREE.MeshDistanceMaterial({alphaTest: 0.5, map: Scene.Map.current.scene.children[i].customDepthMaterial.map});
}, 48);
...
//					lightList[i].dispose();
		...
		for (var i = 0; i < lightList.length; i++)
		{
			if (lightList[i].isDirectionalLight)
			{
				...
				lightList[i].position.set(x, y, z).multiplyScalar(d / 2).add(Scene.Map.current.camera.targetPosition);
...
{
	var vert = await Common.IO.openFile(path + "vert.glsl");
	var frag = await Common.IO.openFile(path + "frag.glsl");
...
/*
const oldInitFunc = Manager.GL.initialize;
Manager.GL.initialize = function()
{
	oldInitFunc();
...
function limitDistance(value)
{
	...
	if (value === 0)
		return h2 * Data.Systems.SQUARE_SIZE;
...
Manager.Plugins.registerCommand(pluginName, "Add hemisphere light", (prop, intensity, colorTop, colorBottom) =>
{
	...
	if (prop > 0)
		Core.ReactionInterpreter.currentObject.properties[prop] = light;
...
Manager.Plugins.registerCommand(pluginName, "Add point light", (prop, id, x, y, z, intensity, color, radius, castShadow) =>
{
...
Manager.Plugins.registerCommand(pluginName, "Add spotlight", (prop, id, x, y, z, intensity, color, angle, distance, castShadow) =>
{
